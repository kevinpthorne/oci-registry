...
# Expects oci-registry to be listening on localhost:8443, for example:
# RUST_LOG=info,actix-web=debug cargo run --features openssl -- --listen 0.0.0.0:8443 --tls-key key.pem --tls-cert cert.pem filesystem --root /tmp/oci-mirror
...
test() {
  ...
  # containerd
  url="https://localhost:8443/v2/$2?ns=$1"
  curl -k -s -o /dev/null -w "  %{http_code}: $url\n" "$url"

  rm -rf /tmp/oci-mirror

  # cri-o
  url="https://localhost:8443/v2/$1/$2"
  curl --cacert test.crt -s -o /dev/null -w "  %{http_code}: $url\n" "$url"

  rm -rf /tmp/oci-mirror

  # Special Docker Hub case falling back to default_ns
  if [ "$1" == "docker.io" ]; then
    url="https://localhost:8443/v2/$2"
    curl --cacert test.crt -s -o /dev/null -w "  %{http_code}: $url\n" "$url"
  fi
}
